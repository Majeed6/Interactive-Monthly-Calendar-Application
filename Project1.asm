#Majeed Issa 1201193
#Mohammad Ali 1200388

#########################################
#here we specified all the needed data in the data section, from prompts to symbols to
#variables and also error messages
.data
file_name:	.asciiz "D:\calendar.txt"
View_Calendar:	.asciiz "1. View the calendar\n"
View_Calendar1:	.asciiz "\nView Calendar:\n1. Per day\n"
View_Calendar2:	.asciiz "2. Per set of days\n"
View_Calendar3:	.asciiz "3. For a given slot in a given day\n"
View_Statistics:	.asciiz "2. View statistics\n"
Add_NewAppointment:	.asciiz "3. Add Appointment\n"
Delete_AnAppointment:	.asciiz "4. Delete Appointment\n"
Exit_And_Save:	.asciiz "\5. Save data and exit\n"
NL: .asciiz "\nTotal Lectures (in hours): "
NO: .asciiz "\nTotal Office Hours (in hours): "
NM: .asciiz "\nTotal Meetings (in hours): "
avg_lect: .asciiz "\nAverage Lectures per day = "
ratio_LtoOH: .asciiz "\nLecture to OH Ratio: "
metting: .asciiz "meeting is Occuring"
lecture: .asciiz "lecture is Occuring"
officehour: .asciiz "Office Hour is Occuring)"
adding_done_msg: .asciiz "\nAppointment Added Succsessfuly !"
deleting_done_msg: .asciiz "\n Appointment Deleted Succsessfuly !"
save_exit: .asciiz "\nEverything has been saved Successfuly !\n"
enter: .asciiz "Enter your choice (1-5): "
enter1: .asciiz "Enter your choice (1-3): "
enterTheDay: .asciiz " Enter a day (1-31): "
View_Calendar2_MSG: .asciiz "Please enter the number of days: "
View_Calendar3_MSG_FirstTime: .asciiz "Enter start time : "
View_Calendar3_MSG_SecondTime: .asciiz "Enter end time : "
read_appointment: .asciiz "Enter type (L, M, O(OH)): "
Lecture: .asciiz " L"
OH: .asciiz " OH"
Meeting: .asciiz " M"
dash: .asciiz "-"
...
brake: .asciiz " Brake"
spaces: .asciiz "\nMenu:\n"
index_error_msg: .asciiz "\nError in file, day should be in proper range (1-31)\n"
error_time_msg:	.asciiz "\nError, time should be in proper range (8-5) \n"
zero_time_error_msg:	.asciiz "\nError, not in proper range (8-5)\n"
order_time_error_msg:	.asciiz "\nError, check time format in file\n"
ElseMSG: .asciiz "\nError, Please choose a valid option!\n"
noDataForThisDay: .asciiz "\nData not found for the given day\n"
OFE: .asciiz "\n Error, Please Check file path\n"
incorrect_Index_MSG: .asciiz "\n  Error, the day must be from 1-31 \n"
incorrect_time: .asciiz "\nError, time should be in proper format (8-5) \n"
incorrect_appointment: .asciiz "\nError, type should be L, M, O(OH) \n"
time_error: .asciiz "\nstarting time should be less than end time \n"
conflict_msg1: .asciiz "\nConflict occured "
conflict_msg2: .asciiz " Confirm Modification (Y(1),N(2))?\n"
delete_type_error: .asciiz "\nMismatch in appointment type"
...
##########################################################################
##########################################################################
#Reading file is done here
#also File reading is being handled in case there was a problem whether the file or the path
...
file_opened_correctly:
	...
	li $v0, 14
	syscall
	# address for file content
	la $s0, file_content	
	la $a0, calendar_arr
	# used for saving the first number that we read
	li $s1, 0	
line_begin:
	lb $a1, 0($s0)
	beqz $a1, file_end
	lb $t1, 1($s0)
	#one digit start time logic is here
	li $t5, ':'	
	and $t4, $t1, $t5
	...
	sll $t0, $a1, 1
	addu $a1, $t0, $t2
	# here we add 1 if number of day has two digits
	addiu $s0, $s0, 1	
	addu $a1, $a1, $t1
one_digit_day:
	addiu $s0, $s0, 1	
	beq $a1, '0', index_error	
	subiu $a1, $a1, 0x31 
...
read_line_content:
	...
	addiu $s0, $s0, 2	
	j line_begin
# this means that it's not the end of line label indecating that the pointer is not at the end of line 
not_EOL:	
	lb $t3, 1($s0)	
	li $t1, '0'
	li $t2, '9'
...
##########################################################################
##########################################################################	
#here's View the callendar choice
FirstChoice:
	#printing options and then taking user input and saving it
...
NumberOFmCells:
	...
	addiu $s0, $s0, 1 # Increment the first choice loop counter
	j NumberOFmCells
#General functions for all choices in first choice
...
Read_Time_Again:	
	...
	li $v0, 4
	syscall
	#####We need to check an error for the times input #####
...
count_days_done:
	...
	li $v0, 1
	syscall
	# if ratio needed as division 
	#mtc1 $s1, $f0	
	#mtc1 $s2, $f1
	#div.s $f2, $f0, $f1	# divide total number of lectures to total number of office hour	
	#mov.s $f12, $f2
	#li $v0, 2
	#syscall
...
time_doneL:
	...
	sb $t6, 0($a3)
	addiu $a3, $a3, 1 # Increment the address of the destination buffer
	#To check if there is any slot after this slot or no to print it
	j LOOP_FOR_A_day
#To find the number of lect Cells
